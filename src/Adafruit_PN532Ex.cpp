...
/*!
    ...
    @section  HISTORY
    v2.2 - Errors in low level PN532 chip access functions fixed.
           Unexplainable calls of delay() removed.
...
*/
/**************************************************************************/
Adafruit_PN532Ex::Adafruit_PN532Ex(uint8_t clk, uint8_t miso, uint8_t mosi, uint8_t ss):
  _clk(clk),
  _miso(miso),
  _mosi(mosi),
  _ss(ss),
...
*/
/**************************************************************************/
Adafruit_PN532Ex::Adafruit_PN532Ex(uint8_t irq, uint8_t reset):
  _clk(0),
  _miso(0),
  _mosi(0),
  _ss(0),
...
*/
/**************************************************************************/
Adafruit_PN532Ex::Adafruit_PN532Ex(uint8_t ss):
  _clk(0),
  _miso(0),
  _mosi(0),
  _ss(ss),
...
*/
/**************************************************************************/
void Adafruit_PN532Ex::begin() {
  // According to AN10609_3 app note from NXP ch.2.5 p.13 and p.23
  // "How to start the PN532C106"
  // the chip starts in Low Vbat mode, i.e. in Card Emulation mode
  // if RF field is present and in Power Down mode otherwise.
  // IRQ is not functional at this time.
  // Special sequence should be executed to go into Normal mode.
  // The sequence depends from the host interface type (SPI or I2C).
  ...
  if (_usingSPI) {
    ...
    if (_hardwareSPI) {
      ...
      // Software SPI pins configuration
      digitalWrite(_ss, HIGH);
      pinMode(_ss, OUTPUT);
  ...
  if (_reset) {
    ...
    delay(1);
    digitalWrite(_reset, HIGH);
    delay(2);  // Small delay required before taking other actions after reset.
               // See timing diagram on page 209 of the datasheet, section 12.23.
  }

  if (_usingSPI) {
    // Wakeup from Power Down via SPI:
    // 1. keep SS low for 1ms
    // 2. send SAM configuration with parameter Mode = 0x01 (Normal)
    //    without use of handshake
    digitalWrite(_ss, LOW);
    delay(1);

    pn532_packetbuffer[0] = PN532_COMMAND_SAMCONFIGURATION;
    pn532_packetbuffer[1] = 0x01; // Normal mode
    pn532_packetbuffer[2] = 0x14; // timeout 50ms * 20 = 1 second - ignored in Normal mode
    pn532_packetbuffer[3] = 0x01; // use IRQ pin!

    // Write the command
    writecommand(pn532_packetbuffer, 4);

    // Do not wait IRQ and do not poll RDY bit, just wait reasonable time
    // Measurements show 0.5 ms, let set it to 2ms
    delay(2);
    ...
    // Do not wait IRQ and do not poll RDY bit, just wait reasonable time
    // Measurements show 0.9 ms, let set it to 5ms
    delay(5);
    ...
    // Wakeup from Power Down via I2C:
    // 1. Send PN532 I2C address (0x48)
    // 2. PN532 will stretch SCL about 1ms
    // To do that just call SAMConfig().
    // Anyway it is the only possible action at this moment.
    SAMConfig();
  }
}
...
*/
/**************************************************************************/
void Adafruit_PN532Ex::PrintHex(const byte * data, const uint32_t numBytes)
{
  uint32_t szPos;
...
*/
/**************************************************************************/
void Adafruit_PN532Ex::PrintHexChar(const byte * data, const uint32_t numBytes)
{
  uint32_t szPos;
  for (szPos=0; szPos < numBytes; szPos++)
  {
...
*/
/**************************************************************************/
uint32_t Adafruit_PN532Ex::getFirmwareVersion(void) {
  uint32_t response;
  ...
  int offset = sizeof(pn532response_firmwarevers);
...
bool Adafruit_PN532Ex::sendCommandCheckAck(uint8_t *cmd, uint8_t cmdlen, uint16_t timeout) {
  uint16_t timer = 0;
...
*/
/**************************************************************************/
bool Adafruit_PN532Ex::writeGPIO(uint8_t pinstate) {
  uint8_t errorbit;
...
*/
/**************************************************************************/
uint8_t Adafruit_PN532Ex::readGPIO(void) {
...
*/
/**************************************************************************/
bool Adafruit_PN532Ex::SAMConfig(void) {
  pn532_packetbuffer[0] = PN532_COMMAND_SAMCONFIGURATION;
  pn532_packetbuffer[1] = 0x01; // normal mode;
...
*/
/**************************************************************************/
bool Adafruit_PN532Ex::setPassiveActivationRetries(uint8_t maxRetries) {
...
bool Adafruit_PN532Ex::readID(uint8_t * uid, uint8_t * uidLength, uint16_t timeout) {
	return readPassiveTargetID(PN532_MIFARE_ISO14443A, uid, uidLength, timeout);
}
...
*/
/**************************************************************************/
bool Adafruit_PN532Ex::readPassiveTargetID(uint8_t cardbaudrate, uint8_t * uid, uint8_t * uidLength, uint16_t timeout) {
...
*/
/**************************************************************************/
bool Adafruit_PN532Ex::inDataExchange(uint8_t * send, uint8_t sendLength, uint8_t * response, uint8_t * responseLength) {
...
*/
/**************************************************************************/
bool Adafruit_PN532Ex::inListPassiveTarget() {
  pn532_packetbuffer[0] = PN532_COMMAND_INLISTPASSIVETARGET;
...
*/
/**************************************************************************/
bool Adafruit_PN532Ex::mifareclassic_IsFirstBlock (uint32_t uiBlock)
{
...
*/
/**************************************************************************/
bool Adafruit_PN532Ex::mifareclassic_IsTrailerBlock (uint32_t uiBlock)
{
  // Test if we are in the small or big sectors
  if (uiBlock < 128)
...
*/
/**************************************************************************/
uint8_t Adafruit_PN532Ex::mifareclassic_AuthenticateBlock (uint8_t * uid, uint8_t uidLen, uint32_t blockNumber, uint8_t keyNumber, uint8_t * keyData)
{
  uint8_t len;
  uint8_t i;
  ...
  if (pn532_packetbuffer[7] != 0x00)
  {
      ...
      Adafruit_PN532Ex::PrintHexChar(pn532_packetbuffer, 12);
    #endif
    return 0;
  }
...
*/
/**************************************************************************/
uint8_t Adafruit_PN532Ex::mifareclassic_ReadDataBlock (uint8_t blockNumber, uint8_t * data)
{
  #ifdef MIFAREDEBUG
  ...
  if (pn532_packetbuffer[7] != 0x00)
  {
      ...
      Adafruit_PN532Ex::PrintHexChar(pn532_packetbuffer, 26);
    #endif
    return 0;
  }
  ...
  /* Display data for debug if requested */
    ...
    Adafruit_PN532Ex::PrintHexChar(data, 16);
  #endif
...
*/
/**************************************************************************/
uint8_t Adafruit_PN532Ex::mifareclassic_WriteDataBlock (uint8_t blockNumber, uint8_t * data)
{
  #ifdef MIFAREDEBUG
...
*/
/**************************************************************************/
uint8_t Adafruit_PN532Ex::mifareclassic_FormatNDEF (void)
{
...
*/
/**************************************************************************/
uint8_t Adafruit_PN532Ex::mifareclassic_WriteNDEFURI (uint8_t sectorNumber, uint8_t uriIdentifier, const char * url)
{
...
*/
/**************************************************************************/
uint8_t Adafruit_PN532Ex::mifareultralight_ReadPage (uint8_t page, uint8_t * buffer)
{
  if (page >= 64)
  {
    #ifdef MIFAREDEBUG
  ...
  /* Display data for debug if requested */
    ...
    Adafruit_PN532Ex::PrintHexChar(buffer, 4);
  #endif
...
*/
/**************************************************************************/
uint8_t Adafruit_PN532Ex::mifareultralight_WritePage (uint8_t page, uint8_t * data)
{
...
*/
/**************************************************************************/
uint8_t Adafruit_PN532Ex::ntag2xx_ReadPage (uint8_t page, uint8_t * buffer)
{
...
*/
/**************************************************************************/
uint8_t Adafruit_PN532Ex::ntag2xx_WritePage (uint8_t page, uint8_t * data)
{
  // TAG Type       PAGES   USER START    USER STOP
  // --------       -----   ----------    ---------
  // NTAG 203       42      4             39
...
*/
/**************************************************************************/
uint8_t Adafruit_PN532Ex::ntag2xx_WriteNDEFURI (uint8_t uriIdentifier, char * url, uint8_t dataLen)
{
...
*/
/**************************************************************************/
bool Adafruit_PN532Ex::readack() {
  uint8_t ackbuff[sizeof(pn532ack)];

  readdata(ackbuff, sizeof(ackbuff));

  if(0 != memcmp((char *)ackbuff, (char *)pn532ack, sizeof(pn532ack))) {
#ifdef PN532DEBUG
   PN532DEBUGPRINT.println(F("No ACK frame received!"));
#endif
    return false;
...
*/
/**************************************************************************/
bool Adafruit_PN532Ex::waitready(uint16_t timeout) {
  uint16_t timer = 0;
  while(!isready()) {
    if (timeout != 0) {
      timer += 1;
...
*/
/**************************************************************************/
void Adafruit_PN532Ex::readdata(uint8_t* buff, uint8_t n) {
  if (_usingSPI) {
    // SPI read.
    #ifdef SPI_HAS_TRANSACTION
      if (_hardwareSPI) SPI.beginTransaction(PN532_SPI_SETTING);
...
*/
/**************************************************************************/
void Adafruit_PN532Ex::writecommand(uint8_t* cmd, uint8_t cmdlen) {
  if (_usingSPI) {
    // SPI command write.
    uint8_t checksum;
...
*/
/**************************************************************************/
void Adafruit_PN532Ex::spi_write(uint8_t c) {
  if (_hardwareSPI) {
    // Hardware SPI write.
    SPI.transfer(c);
  }
...
*/
/**************************************************************************/
uint8_t Adafruit_PN532Ex::spi_read(void) {
  int8_t i, x;
  x = 0;
