 ...
 */
void MFRC522Ex::PCD_WriteRegister(	PCD_Register reg,	///< The register to write to. One of the PCD_Register enums.
 ...
 * The interface is described in the datasheet section 8.1.2.
 */
byte MFRC522Ex::PCD_ReadRegister(	PCD_Register reg	///< The register to read from. One of the PCD_Register enums.
								) {
	byte value;
	_spiClass->beginTransaction(_spiSettings);	// Set the settings to work with SPI bus
	digitalWrite(_chipSelectPin, LOW);			// Select slave
 ...
 * The interface is described in the datasheet section 8.1.2.
 */
void MFRC522Ex::PCD_ReadRegister(	PCD_Register reg,	///< The register to read from. One of the PCD_Register enums.
 ...
 */
void MFRC522Ex::PCD_SetRegisterBitMask(	PCD_Register reg,	///< The register to update. One of the PCD_Register enums.
 ...
 */
void MFRC522Ex::PCD_ClearRegisterBitMask(	PCD_Register reg,	///< The register to update. One of the PCD_Register enums.
 ...
 */
MFRC522Ex::StatusCode MFRC522Ex::PCD_CalculateCRC(	byte *data,		///< In: Pointer to the data to transfer to the FIFO for CRC calculation.
 ...
 * Initializes the MFRC522 chip.
 */
void MFRC522Ex::PCD_Init(	byte chipSelectPin,		///< Arduino pin connected to MFRC522's SPI slave select input (Pin 24, NSS, active low)
 ...
 */
byte MFRC522Ex::PCD_GetAntennaGain() {
 ...
 */
void MFRC522Ex::PCD_SetAntennaGain(byte mask) {
 ...
 */
bool MFRC522Ex::PCD_PerformSelfTest() {
...
void MFRC522Ex::PCD_SoftPowerDown(){//Note : Only soft power down mode is available throught software
...
void MFRC522Ex::PCD_SoftPowerUp(){
	byte val = PCD_ReadRegister(CommandReg); // Read state of the command register 
 ...
 * @return STATUS_OK on success, STATUS_??? otherwise.
 */
MFRC522Ex::StatusCode MFRC522Ex::PCD_TransceiveData(	byte *sendData,		///< Pointer to the data to transfer to the FIFO.
 ...
 * @return STATUS_OK on success, STATUS_??? otherwise.
 */
MFRC522Ex::StatusCode MFRC522Ex::PCD_CommunicateWithPICC(	byte command,		///< The command to execute. One of the PCD_Command enums.
	...
	if (backData && backLen && checkCRC) {
		...
		MFRC522Ex::StatusCode status = PCD_CalculateCRC(&backData[0], *backLen - 2, &controlBuffer[0]);
		if (status != STATUS_OK) {
			return status;
		}
 ...
 * @return STATUS_OK on success, STATUS_??? otherwise.
 */
MFRC522Ex::StatusCode MFRC522Ex::PICC_RequestA(	byte *bufferATQA,	///< The buffer to store the ATQA (Answer to request) in
 ...
 * @return STATUS_OK on success, STATUS_??? otherwise.
 */
MFRC522Ex::StatusCode MFRC522Ex::PICC_WakeupA(	byte *bufferATQA,	///< The buffer to store the ATQA (Answer to request) in
											byte *bufferSize	///< Buffer size, at least two bytes. Also number of bytes returned if STATUS_OK.
										) {
 ...
 * @return STATUS_OK on success, STATUS_??? otherwise.
 */ 
MFRC522Ex::StatusCode MFRC522Ex::PICC_REQA_or_WUPA(	byte command, 		///< The command to send - PICC_CMD_REQA or PICC_CMD_WUPA
												byte *bufferATQA,	///< The buffer to store the ATQA (Answer to request) in
												byte *bufferSize	///< Buffer size, at least two bytes. Also number of bytes returned if STATUS_OK.
											) {
	byte validBits;
	MFRC522Ex::StatusCode status;
 ...
 * @return STATUS_OK on success, STATUS_??? otherwise.
 */
MFRC522Ex::StatusCode MFRC522Ex::PICC_Select(	Uid *uid,			///< Pointer to Uid struct. Normally output, but can also be used to supply a known UID.
 ...
 * @return STATUS_OK on success, STATUS_??? otherwise.
 */ 
MFRC522Ex::StatusCode MFRC522Ex::PICC_HaltA() {
	MFRC522Ex::StatusCode result;
	byte buffer[4];
 ...
 */
MFRC522Ex::StatusCode MFRC522Ex::PCD_Authenticate(byte command,		///< PICC_CMD_MF_AUTH_KEY_A or PICC_CMD_MF_AUTH_KEY_B
 ...
 */
void MFRC522Ex::PCD_StopCrypto1() {
 ...
 * @return STATUS_OK on success, STATUS_??? otherwise.
 */
MFRC522Ex::StatusCode MFRC522Ex::MIFARE_Read(	byte blockAddr, 	///< MIFARE Classic: The block (0-0xff) number. MIFARE Ultralight: The first page to return data from.
 ...
 * @return STATUS_OK on success, STATUS_??? otherwise.
 */
MFRC522Ex::StatusCode MFRC522Ex::MIFARE_Write(	byte blockAddr, ///< MIFARE Classic: The block (0-0xff) number. MIFARE Ultralight: The page (2-15) to write to.
 ...
 * @return STATUS_OK on success, STATUS_??? otherwise.
 */
MFRC522Ex::StatusCode MFRC522Ex::MIFARE_Ultralight_Write(	byte page, 		///< The page (2-15) to write to.
 ...
 * @return STATUS_OK on success, STATUS_??? otherwise.
 */
MFRC522Ex::StatusCode MFRC522Ex::MIFARE_Decrement(	byte blockAddr, ///< The block (0-0xff) number.
 ...
 * @return STATUS_OK on success, STATUS_??? otherwise.
 */
MFRC522Ex::StatusCode MFRC522Ex::MIFARE_Increment(	byte blockAddr, ///< The block (0-0xff) number.
 ...
 * @return STATUS_OK on success, STATUS_??? otherwise.
 */
MFRC522Ex::StatusCode MFRC522Ex::MIFARE_Restore(	byte blockAddr ///< The block (0-0xff) number.
											) {
 ...
 * @return STATUS_OK on success, STATUS_??? otherwise.
 */
MFRC522Ex::StatusCode MFRC522Ex::MIFARE_TwoStepHelper(	byte command,	///< The command to use
 ...
 * @return STATUS_OK on success, STATUS_??? otherwise.
 */
MFRC522Ex::StatusCode MFRC522Ex::MIFARE_Transfer(	byte blockAddr ///< The block (0-0xff) number.
											) {
	MFRC522Ex::StatusCode result;
	byte cmdBuffer[2]; // We only need room for 2 bytes.
 ...
 * @return STATUS_OK on success, STATUS_??? otherwise.
  */
MFRC522Ex::StatusCode MFRC522Ex::MIFARE_GetValue(byte blockAddr, int32_t *value) {
	MFRC522Ex::StatusCode status;
	byte buffer[18];
 ...
 * @return STATUS_OK on success, STATUS_??? otherwise.
 */
MFRC522Ex::StatusCode MFRC522Ex::MIFARE_SetValue(byte blockAddr, int32_t value) {
	byte buffer[18];
 ...
 * @return STATUS_OK on success, STATUS_??? otherwise.
 */
MFRC522Ex::StatusCode MFRC522Ex::PCD_NTAG216_AUTH(byte* passWord, byte pACK[]) //Authenticate with 32bit password
{
 ...
 * @return STATUS_OK on success, STATUS_??? otherwise.
 */
MFRC522Ex::StatusCode MFRC522Ex::PCD_MIFARE_Transceive(	byte *sendData,		///< Pointer to the data to transfer to the FIFO. Do NOT include the CRC_A.
 ...
 */
const __FlashStringHelper *MFRC522Ex::GetStatusCodeName(MFRC522Ex::StatusCode code	///< One of the StatusCode enums.
										) {
	return MFRC522DebugEx::GetStatusCodeName(code);
 ...
 * @return PICC_Type
 */
MFRC522Ex::PICC_Type MFRC522Ex::PICC_GetType(byte sak		///< The SAK byte returned from PICC_Select().
										) {
 ...
 * @return const __FlashStringHelper *
 */
const __FlashStringHelper *MFRC522Ex::PICC_GetTypeName(PICC_Type piccType	///< One of the PICC_Type enums.
													) {
	return MFRC522DebugEx::PICC_GetTypeName(piccType);
 ...
 */
void MFRC522Ex::PCD_DumpVersionToSerial() {
 ...
 */
void MFRC522Ex::PICC_DumpToSerial(Uid *uid	///< Pointer to Uid struct returned from a successful PICC_Select().
								) {
	MIFARE_Key key;
 ...
 */
void MFRC522Ex::PICC_DumpDetailsToSerial(Uid *uid	///< Pointer to Uid struct returned from a successful PICC_Select().
									) {
	// UID
 ...
 * On success the PICC is halted after dumping the data.
 */
void MFRC522Ex::PICC_DumpMifareClassicToSerial(	Uid *uid,			///< Pointer to Uid struct returned from a successful PICC_Select().
 ...
 */
void MFRC522Ex::PICC_DumpMifareClassicSectorToSerial(Uid *uid,			///< Pointer to Uid struct returned from a successful PICC_Select().
 ...
 */
void MFRC522Ex::PICC_DumpMifareUltralightToSerial() {
	MFRC522Ex::StatusCode status;
	byte byteCount;
	byte buffer[18];
	byte i;
 ...
 */
void MFRC522Ex::MIFARE_SetAccessBits(	byte *accessBitBuffer,	///< Pointer to byte 6, 7 and 8 in the sector trailer. Bytes [0..2] will be set.
 ...
 * @return bool
 */
bool MFRC522Ex::PICC_IsNewCardPresent() {
	byte bufferATQA[2];
	...
	MFRC522Ex::StatusCode result = PICC_RequestA(bufferATQA, &bufferSize);
	//Serial.print("INCP: ");
	//Serial.println(GetStatusCodeName(result));
	return (result == STATUS_OK || result == STATUS_COLLISION);
} // End PICC_IsNewCardPresent()

bool MFRC522Ex::PICC_IsNewCardPresentWakeup() {
	byte bufferATQA[2];
	byte bufferSize = sizeof(bufferATQA);
	...
	MFRC522Ex::StatusCode result = PICC_WakeupA(bufferATQA, &bufferSize);
	//Serial.print("INCP: ");
	//Serial.println(GetStatusCodeName(result));
	return (result == STATUS_OK || result == STATUS_COLLISION);
} // End PICC_IsNewCardPresent()
 ...
 * @return bool
 */
bool MFRC522Ex::PICC_ReadCardSerial() {
	MFRC522Ex::StatusCode result = PICC_Select(&uid);
	return (result == STATUS_OK);
} // End 

// NFC Interface Implementationn

void MFRC522Ex::begin() {
	SPI.begin();
	PCD_Init();
	//PCD_SetAntennaGain(RxGain_33dB);
}

NFCInterface::NFCChipType MFRC522Ex::getChipType() {
	return NFCInterface::NFC_MFRC522;
}

void MFRC522Ex::getChipTypeString(char * str) {
...
uint32_t MFRC522Ex::getFirmwareVersion() {
...
void MFRC522Ex::getFirmwareVersionString(char * str) {
	uint32_t nfcVersionData = getFirmwareVersion();
...
bool MFRC522Ex::SAMConfig() {
	return true;
}

bool MFRC522Ex::readID(uint8_t * uidVal, uint8_t * uidLength, uint16_t timeout) {
	PICC_ReadCardSerial(); // Always fails
	if (PICC_IsNewCardPresentWakeup()) {
		if (PICC_ReadCardSerial()) {
			memcpy(uidVal, uid.uidByte, uid.size);
			*uidLength = uid.size;
			return true;
		}
		else {
			Serial.println("Read serial failed.");
		}
	}
	yield();
	return false;
...
uint8_t MFRC522Ex::ntag2xx_ReadPage(uint8_t page, uint8_t * buffer) {
	uint8_t tempBuf[18] = {0};
	byte tempBufSize = sizeof(tempBuf);
	MFRC522Ex::StatusCode status = MIFARE_Read(page, tempBuf, &tempBufSize);
    if (status == MFRC522Ex::STATUS_OK)
	{
		memcpy(buffer, tempBuf, 4);
...
void MFRC522Ex::ntag2xx_FinishedReading() {
	Serial.println("Stopping card crypto");
	PICC_HaltA();
	PCD_StopCrypto1();
}

uint8_t MFRC522Ex::ntag2xx_WritePage(uint8_t page, uint8_t * data) {
	const byte fixedSize = 4;
	MFRC522Ex::StatusCode status = MIFARE_Ultralight_Write(page, data, fixedSize);
	if (status == MFRC522Ex::STATUS_OK)
	{
